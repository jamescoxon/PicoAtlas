...
int count = 0, j, q, n, hour = 0 , minute = 0 , second = 0, battV = 0, dataloop = 10, gpsonoff = 0;
...
{  
  if (f_wdt==1) {  // wait for timed out watchdog / flag is set when a watchdog timeout occurs
   ...
   if((count % 50) == 0 && gpsonoff == 1 && battV < 750) {
     gpsOff();
   }
   
   if((count % 200) == 0) {
     if(gpsonoff == 0 && battV > 740){
       gpsOn();
       dataloop = 5;
     }
   }
   ...
   Serial.println("$PUBX,00*33"); //Poll GPS
    ...
    n=sprintf (superbuffer, "PICO,%d,%02d:%02d:%02d,%ld,%ld,%ld,%d,%d", count, hour, minute, second, lat, lon, ialt, battV, gpsonoff);
