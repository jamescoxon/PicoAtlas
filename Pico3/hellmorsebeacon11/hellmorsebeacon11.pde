...
int count = 0, j, q, n, hour = 0 , minute = 0 , second = 0, battV = 0, dataloop = 10, gpsonoff = 0, gpsAwake = 0, solarV = 0;
...
{  
  if (f_wdt==1) {  // wait for timed out watchdog / flag is set when a watchdog timeout occurs
   ...
   Serial.println("$PUBX,00*33"); //Poll GPS
    ...
    n=sprintf (superbuffer, "PICO,%d,%02d:%02d:%02d,%ld,%ld,%ld,%d,%d,%d%d", count, hour, minute, second, lat, lon, ialt, battV, solarV, gpsonoff, gpsAwake);
