...
long int ialt = 123;
int numbersats = 99, battV = 0, solarV = 0;
float flat, flon;
...
void loop(){
  ...
  if (f_wdt==1) {  // wait for timed out watchdog / flag is set when a watchdog timeout occurs
    ...
    if((count % loopnumber) == 0) {
      ...
      wdt_reset();
      if (gpsonoff == 1){
        n=sprintf (superbuffer, "$$PICO,%d,%02d:%02d:%02d,%s,%s,%ld,%d,%d;%d;%d", count, hour, minute, second, latbuf, lonbuf, ialt, numbersats, navmode, battV, solarV);
      }
      else {
        n=sprintf (superbuffer, "$$PICO,%d,%d,%d,%d", count, navmode, battV, solarV);
      }
